...
class BaseTrainer(object):
    ...
    def metrics_print(self, result_file):
        ...
        true_labels = df['true'].astype(int).to_numpy()
        predicted_labels = np.array([int(predict) for predict in df['predict']])
        pred_scores = np.array([ast.literal_eval(probs) for probs in df['probs']])
        ...
        if num_labels == 1:
            ...
            #auc = roc_auc_score(true_labels, pred_scores, multi_class='ovr', average='macro')
            pr_auc = average_precision_score(true_labels, pred_scores[:, 1])
            # true_onehot = np.zeros_like(pred_scores)
            # true_onehot[np.arange(len(true_labels)), true_labels] = 1
            #
            # prauc_per_class = [average_precision_score(true_onehot[:, i], pred_scores[:, i]) for i in
            #                    range(4)]
            # pr_auc = np.mean(prauc_per_class)
            ...
            results_str = [
                f"AUROC = {auc:.4f}, PRAUC = {pr_auc:.4f}, ACC = {acc:.4f}, F1-score = {f1:.4f}"
            ]
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, rank):
        ...
        for batch_idx, (images_id, images, labels, text_features) in enumerate(
            ...
            output = self.model(images, text_features, mode='train')
            logits = output[0]  # [batch_size, num_classes]
