...
class Encoder(nn.Module):
    ...
    def forward(self, x, text_features, gcn_query, mask, patch_fine_pref):
        for i, layer in enumerate(self.layers):
            x = layer(x, text_features, gcn_query, mask,patch_fine_pref)
        return self.norm(x)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, self_attn, self_attn2, self_attn3, feed_forward, dropout, gamma_init=0.3):
        ...
        self.gcn_prompt_learner = GcnPromptLearner()
        self.gamma = nn.Parameter(torch.tensor(gamma_init, dtype=torch.float32))

    def calibrate_similarity(self, sim_matrix, patch_fine_pref):
        """
        sim_matrix:      [B, N, 3M]
        patch_fine_pref: [B, N, 3M]
        """

        gate = torch.sigmoid(self.gamma * patch_fine_pref)
        #sim_calibrated = gate * (patch_fine_pref * sim_matrix)  + sim_matrix
        sim_calibrated = sim_matrix+self.gamma*patch_fine_pref
        return sim_calibrated
    def build_sim_graph(self, patch_feats, sim_matrix,patch_fine_pref,k_num=4, alpha=0.5):

        batch_size, num_patches, num_texts = sim_matrix.shape
        edge_indices, edge_weights = [], []

        for b in range(batch_size):
            # text similarity
            patch_sim = sim_matrix[b]  # [N, M]
            #patch_sim_norm = F.normalize(patch_sim, dim=-1)
            sim_calibrated = self.calibrate_similarity(sim_matrix, patch_fine_pref)  # [B, N, 3M]

            patch_sim = sim_calibrated[b]  # [N, 3M]
            patch_sim_norm = F.normalize(patch_sim, dim=-1)
            A_text = patch_sim_norm @ patch_sim_norm.T  # [N, N]

            # visual similarity
            patch_feat = patch_feats[b]  # [N, D]
            patch_feat_norm = F.normalize(patch_feat, dim=-1)
            A_vis = patch_feat_norm @ patch_feat_norm.T  # [N, N]

            # final similarity
            A_final = alpha * A_text + (1 - alpha) * A_vis

            A_cpu = A_final.detach().cpu()

            topk_values, topk_indices = torch.topk(A_cpu, k=k_num + 1, dim=-1)

            topk_values = topk_values.to(A_final.device)
            topk_indices = topk_indices.to(A_final.device)
            #topk_values, topk_indices = torch.topk(A_final, k=k_num + 1, dim=-1)
            source_nodes = torch.arange(num_patches, device=A_final.device).repeat_interleave(k_num)
    ...
    def forward(self, x, text_features, gcn_query=None, mask=None,patch_fine_pref=None):
        x0=x
        ...
        sim_matrix = torch.bmm(x_norm, text_norm.transpose(1, 2))  # [B, N, M]
        edge_index, edge_weight = self.build_sim_graph(
            patch_feats=x_norm,
            sim_matrix=sim_matrix,
            patch_fine_pref=patch_fine_pref,
            k_num=4,
            alpha=0.5
        )

        gcn_query = self.gcn_prompt_learner(x0.squeeze(0), edge_index.cuda(), edge_weight.cuda())
        gcn_query = gcn_query.unsqueeze(0)
...
#                              use_pred=use_pred).to(self.device)
    ...
    #     return out
    # def forward(self, edges, node_features, edge_weight=None):
    #     data = Data(x=node_features.to(self.device),
    #                 edge_index=edges.to(self.device),
    #                 edge_weight=None if edge_weight is None else edge_weight.to(self.device))
    #     out = self.gnn_model(data.x, data.edge_index, data.edge_weight)
    #     return out
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args):
        ...
        # for p in self.image_classifier.parameters():
        #     p.requires_grad = False

        with open('./TCGA-NSCLC/final_prototypes.pkl',
                  'rb') as f:
            data = pickle.load(f)
        fused_prototypes = torch.tensor(data['fused_prototypes'], dtype=torch.float32)
        proto_coarse_text_sim = torch.tensor(
            data['sim_matrix'], dtype=torch.float32
        )  # shape: [k, m]
        self.prototypes = nn.Parameter(fused_prototypes, requires_grad=False)
        self.proto_coarse_text_sim = nn.Parameter(proto_coarse_text_sim, requires_grad=False)
        self.prototype_proj = nn.Linear(2048, 768, bias=False)
    ...
    def compute_prototype_scores(self, att_feats):
        """
        Compute cosine similarity scores between patches and dataset-level prototypes.

        Args:
            att_feats: [batch_size, patch_num, feat_dim]
            self.prototypes: [num_prototypes, feat_dim]
    ...
    def select_patches_with_prototype(self, att_feats, patch_scores, k_sample):
        """
        Select top-k patches based on prototype similarity scores.
        """
        topk_values, topk_indices = torch.topk(patch_scores, k_sample, dim=1)  # [B, k]
        sorted_indices, _ = torch.sort(topk_indices, dim=1)  # 保证顺序一致
        selected_feats = torch.gather(
            att_feats, 1, sorted_indices.unsqueeze(-1).expand(-1, -1, att_feats.size(2))
        )
        return selected_feats

    def compute_patch_fine_preference(self, selected_feats):
        """
        selected_feats: [B, n, D]
        returns:
            patch_fine_pref: [B, n, 3m]
        """
        patch_norm = F.normalize(selected_feats, dim=-1)

        proto_768 = self.prototype_proj(self.prototypes)
        proto_norm = F.normalize(proto_768, dim=-1)

        # [B, n, k]
        proto_patch_sim = torch.matmul(patch_norm, proto_norm.T)

        # [B, n, m]
        patch_coarse_pref = torch.matmul(
            proto_patch_sim, self.proto_coarse_text_sim
        )

        # [B, n, 3m]
        patch_fine_pref = patch_coarse_pref.repeat_interleave(3, dim=-1)

        return patch_fine_pref
    ...
    def _prepare_feature_mesh(self, att_feats, att_masks=None, meshes=None, tokens=pad_tokens):
        att_feats = tokens(att_feats)
    ...
    def forward(self, att_feats, text_features, att_masks=None):
        ...
        if patch_num > self.args.k_sample:
            patch_scores = self.compute_prototype_scores(att_feats)
            selected_feats = self.select_patches_with_prototype(att_feats, patch_scores, self.args.k_sample)
        else:
            selected_feats = att_feats
        patch_fine_pref=self.compute_patch_fine_preference(selected_feats)
        ...
        t1 = time.time()
        out = self.model(selected_feats, text_features, None, att_masks, patch_fine_pref)
        t2 = time.time()
        return out
